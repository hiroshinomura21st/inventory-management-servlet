...
<main>
    <article class="registration">
        ...
        <form action="CreateServlet" method="post" class="registration-form">
            <div>
                <label for="item_code">商品コード</label>
                <input type="text" id="item_code" name="itemCode" min="0" max="100000000" required>

                <label for="item_name">商品名</label>
                <input type="text" id="item_name" name="name" maxlength="50" required>

                <label for="price">単価</label>
                <input type="number" id="price" name="price" min="0" max="100000000" required>

                <label for="stock_number">在庫数</label>
                <input type="number" id="stock_number" name="stockNum" min="0" max="100000000" required>

                <label for="supplier_code">仕入先コード</label>
                <select id="supplier_code" name="supplierCode"  required>
                    <option disabled selected value>選択してください</option>
                    <c:forEach var="supplier" items="${supplierList}">
                       <option><c:out value="${supplier.supplierCode}" /></option>
                    </c:forEach>
                </select>
            </div>
